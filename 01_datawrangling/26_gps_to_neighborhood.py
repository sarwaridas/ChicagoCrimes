...
def ispointinside(p, poly):
    ln = len(poly)
    return is_odd(sum(rayintersectseg(p, edge) for edge in poly.edges))
...
def get_all_neighborhoods():
    ...
    for shape_idx in range(len(d["features"])):
        ...
        for coordinate_idx in range(
            len(d["features"][shape_idx]["geometry"]["coordinates"][0]) - 1
        ):
            lon_1 = d["features"][shape_idx]["geometry"]["coordinates"][0][
                coordinate_idx
            ][0]
            lat_1 = d["features"][shape_idx]["geometry"]["coordinates"][0][
                coordinate_idx
            ][1]

            lon_2 = d["features"][shape_idx]["geometry"]["coordinates"][0][
                coordinate_idx + 1
            ][0]
            lat_2 = d["features"][shape_idx]["geometry"]["coordinates"][0][
                coordinate_idx + 1
            ][1]
...
for i in range(len(locations)):
    neighborhood_loc = find_neighborhood(
        locations["LONGITUDE"][i], locations["LATITUDE"][i], all_neighborhoods
    )
    neighborhood.append(neighborhood_loc)

locations["CCA"] = pd.DataFrame(neighborhood)

locations.to_csv("Locations_withCCA.csv")
