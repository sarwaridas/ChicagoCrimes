...
def download_shapefiles():
    ...
    comm_area_url = (
        "https://data.cityofchicago.org/download/" "9wp7-iasj/application/zip"
    )
    try:
        os.mkdir("data")
    except OSError:
...
def get_community_area_coords():
    ...
    if len(shapefiles) != 1:
        raise ValueError(
            "There is more than one shapefile matching the" "glob 'data/*.shp'"
        )
    ...
    for i, (shape, rec) in enumerate(zip(shapes, recs)):
        ...
        os.system(
            "gdaltransform -s_srs '+proj=tmerc +lat_0=36.66666666666666 "
            "+lon_0=-88.33333333333333 +k=0.9999749999999999 "
            "+x_0=300000.0000000001 +y_0=0 +ellps=GRS80 "
            "+towgs84=0,0,0,0,0,0,0 +units=us-ft +no_defs' "
            " -t_srs epsg:4326 < " + temp_out_filename + " > " + temp_in_filename
        )
